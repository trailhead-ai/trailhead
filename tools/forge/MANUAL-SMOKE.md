...
**Implication:** forge's agents register as `forge:<name>` once installed + the
session restarts. P3's "host the 13 general dev agents in forge" is therefore
unblocked — agents migrated into `plugins/forge/agents/` are dispatchable as
`forge:<name>`.

**Forge-specific confirmation (manual — run once):** install forge and dispatch
a forge agent in a fresh session (steps below). Record PASS/FAIL here.

| Date | Who | forge install + `forge:<name>` dispatch | Notes |
|------|-----|------------------------------------------|-------|
...
### 4. Confirm a forge agent dispatches as a subagent_type

Dispatch a cheap forge helper via the `Agent` tool with
`subagent_type: "forge:doc-finder"` (or ask Claude to "dispatch the forge
doc-finder agent") on a trivial lookup.

Pass criteria: the agent is found and dispatched (it appears in the available
subagent types as `forge:doc-finder`) and returns a result.
...
| `forge:doc-finder` not a known subagent_type | Plugin not installed, session not restarted, or agent missing valid frontmatter at `plugins/forge/agents/doc-finder.md` |
