...
description: >
  ...
  mergeable, then merge in dependency order. Use for /watch-pr, "watch the PRs",
  "monitor CI", "wait for checks", or when PRs are open and need monitoring.
---

# PR Watch

**Implementation:** this skill is a thin wrapper around the `watch-pr` subagent
...
so watch-pr's merge/blocker notifications never reach the user.
...
- The agent infers scripts from context; SCRIPTS_DIR is `<forge_plugin_root>/scripts/`.
