...
description: >
  ...
  then watch CI until all PRs are mergeable. Use for /create-pr, "create a PR",
...
## Step 3 — Dispatch `pr-updater` for the push tail

Once review is clean, dispatch the `pr-updater` subagent (Sonnet/medium) to handle the
mechanical tail: detect repos, preflight, push, open PRs, link siblings, write prs.json
...
## Step 4 — Launch watch-pr from the top-level session
