...
description: |
  ...
  Escalates to diagnose-preview on regression; exits clean when no health
  command is configured (inert by default — D-3).

  Good fits:
  - Receiving a `post_merge_handoff` JSON marker from watch-pr's completion summary
  - Direct invocation via the forge watch-preview skill for manual soak runs
  - Any post-merge flow that needs a thin generic health gate

  Bad fits:
  - Before merges have completed (dispatch watch-pr first)
...
background session after watch-pr completes. Your job is to run the configured
health command, report the result, and escalate if the deploy regressed.
...
`SCRIPTS_DIR` is `<forge_plugin_root>/scripts/` — resolve from context or pass as env/input.
...
Log: `Watch-preview: soak n/a — no health command configured.` Exit clean.
...
Log: `Watch-preview: soak clean.` Exit clean.

**Exit nonzero — health command failed or timed out (R-4 one-shot escalate):**

Escalate by dispatching the `diagnose-preview` subagent (via the Agent tool):

```
Agent(
  subagent_type: "diagnose-preview",
  prompt: {
...
```
**Watch-preview result:** clean | n/a (no health command) | escalated
