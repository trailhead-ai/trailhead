...
REGEN_HOOK = PLUGIN_ROOT / "hooks" / "pre-commit-regen-indices.sh"
...
def _run_main(vault: Path) -> int:
    ...
    """
    mod = load_regen()
    return mod.main(["regenerate_indices", str(vault)])
...
class TestBasicGeneration:
    ...
    def test_vault_not_found_returns_nonzero(self, tmp_path):
        ...
        mod = load_regen()
        rc = mod.main(["regenerate_indices", str(nonexistent)])
        assert rc != 0
...
def _run_regen_hook(cwd: Path, *, plugin_root: Path = PLUGIN_ROOT) -> subprocess.CompletedProcess:
    """Invoke the regen hook directly with cwd set (mirroring how git runs pre-commit
...
class TestRegenHookDerivesVaultFromGitRoot:
    ...
    def test_regen_targets_git_root_with_non_default_vault(self, tmp_path):
        """The hook regenerates AND stages the index for the committed (git) vault at a
        NON-DEFAULT path, proving it targets the committed repo — not a config default.
        A decoy vault at a different path is left untouched.
        """
        # Decoy "config default" vault at a DIFFERENT path (never committed).
        decoy = tmp_path / "decoy-default-vault"
        (decoy / "deferred").mkdir(parents=True)
        _write_deferred(decoy, "synth-decoy-item")

        # The committed vault at a NON-DEFAULT, arbitrary tmp path.
        committed = _git_vault(tmp_path, "deferred")
        _write_deferred(committed, "synth-committed-record")

        result = _run_regen_hook(committed)
        assert result.returncode == 0, (
            f"hook failed: rc={result.returncode} stderr={result.stderr!r}"
        )

        # Committed vault's index was regenerated and mentions the committed note.
        committed_index = committed / "deferred" / "_index.md"
        assert committed_index.exists(), "committed vault index was not regenerated by the hook"
        assert "synth-committed-record" in committed_index.read_text()

        # The index was STAGED (scoped `git add`) so it rides along with the commit.
        staged = subprocess.run(
            ["git", "-C", str(committed), "diff", "--cached", "--name-only"],
            capture_output=True,
            text=True,
            check=True,
        )
        assert "deferred/_index.md" in staged.stdout.split(), (
            "regenerated index was not staged by the hook"
        )

        # The decoy vault at the other path was NOT touched.
        assert not (decoy / "deferred" / "_index.md").exists(), (
            "decoy vault index was regenerated — hook must target only the git-derived vault"
        )

    def test_regen_hook_exits_zero_when_not_a_git_repo(self, tmp_path):
        """When `git rev-parse` fails (cwd is not a repo) the hook exits 0 and does NOT
        block the commit — the never-block-a-commit invariant (Council Critical #1)."""
        non_repo = tmp_path / "not-a-git-repo"
        non_repo.mkdir()

        result = _run_regen_hook(non_repo)
        assert result.returncode == 0, (
            f"hook must exit 0 when git rev-parse fails; got rc={result.returncode}, "
            f"stderr={result.stderr!r}"
        )
        assert "not a git repo" in result.stderr, (
            f"expected the git-derivation skip message on stderr; got {result.stderr!r}"
        )
