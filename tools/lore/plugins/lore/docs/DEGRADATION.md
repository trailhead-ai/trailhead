...
|---|---|---|---|
| `design_mockup` — UI mockup generation (brainstorm, step 4) | With forge installed, brainstorm dispatches the forge `artist` agent (the default provider) to render the design. Without a provider, UI direction is settled in conversation only and no rendered design artifact is produced | When dispatched, the `artist` returns per-screen HTML + an `index.md`; when no provider is available (forge not installed) or design work isn't applicable, "the mockup step is skipped" surfaces in step 4 and the skill continues with a verbal UI description | Install forge — its `artist` agent is the wired default provider. Or configure your own design-mockup tool and reference it in a local skill or extension; see your plugin's extend guide |
...
| `forge/plan` — Planning skill for implementation slicing (brainstorm, exit gate) | Brainstorming completes and the spec is frozen; the handoff to implementation planning cannot proceed automatically | "the `plan` skill lives in the forge plugin — install forge to continue into planning, or write your implementation slices directly from this spec" printed at the exit gate | Install the forge plugin; the `plan` skill becomes available as `/forge:plan` |
