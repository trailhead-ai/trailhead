...
Dispatch **one** `lore:librarian` subagent with the gathered paths. If `lore:librarian` is not configured, write the reflection yourself from the gathered paths; note in the output that the synthesis pass was skipped and results may be shallower.
