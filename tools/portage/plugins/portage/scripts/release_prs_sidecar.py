...
def _parse_pr_token(token: str) -> dict[str, str]:
    ...
    if not rest:
        raise ValueError(
            f"release_prs_sidecar: --pr must be <repo>:<pr_number>:<url>:<branch>, got: {token!r}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Read or write the portage-owned prs.json sidecar.")
