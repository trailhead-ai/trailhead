"""Every shipped portage skill must be registrable by Claude Code.

A SKILL.md only registers as an invocable `/portage:<name>` command if it opens
...
Unique basename so it does not collide with craft's/lore's same-named test.
"""
...
SKILLS_DIR = Path(__file__).resolve().parents[1] / "plugins" / "portage" / "skills"
...
def test_all_four_portage_skills_present():
    names = {p.parent.name for p in _skill_files()}
    expected = {"open", "merge", "update", "monitor"}
    missing = expected - names
    assert not missing, f"expected portage skills missing: {sorted(missing)}"
...
def _frontmatter(skill_md: Path) -> str:
    ...
    assert text.startswith("---\n"), (
        ...
        "or Claude Code will not register it as a /portage: command"
    )
    end = text.find("\n---", 3)
...
def test_skill_name_matches_dir(skill_md: Path):
    ...
    assert name == skill_md.parent.name, (
        ...
        f"skill dir name {skill_md.parent.name!r} (registers as /portage:{skill_md.parent.name})"
    )
