...
def cmd_help(args: list[str]) -> None:
    ...
    print(
        ...
        "\n"
        "Usage:\n"
        "  camp new <slug> [--no-attach] [--no-session] [--launch] [--json]\n"
        ...
        "  camp list --all-groups|-g [--json]\n"
        "                                    List every configured group's\n"
        "                                    worktrees in one answer, merged\n"
        "  camp list --host <name> [--json]\n"
        "                                    List every group's worktrees on one\n"
        "                                    declared remote machine, over SSH;\n"
        "                                    refuses alongside --group and\n"
        "                                    --all-groups\n"
        "  camp list --all-hosts|-a [--json]\n"
        "                                    List this resolved group's worktrees\n"
        "                                    on every declared machine plus this\n"
        "                                    one, merged into one answer; -ag\n"
        "                                    (bundled -a -g) widens every group\n"
        "                                    too. Refuses alongside --host\n"
        ...
        "  camp launch <slug> [--json]       Start a detached harness session in a workspace\n"
        "  camp launch --dir <path> --group <name> [--json]\n"
        "                                    Start one at a directory the group's\n"
        "                                    [launch] roots allowlist permits; --group\n"
        "                                    must be named explicitly (the allowlist is\n"
        "                                    the boundary, so it never comes from cwd)\n"
        "  camp launch --resume <ref> [--group <name>] [--json]\n"
        "                                    Re-enter a dead session by unambiguous\n"
        "                                    prefix of its name or id, from any cwd. A\n"
        "                                    session rooted in a camp workspace needs no\n"
        "                                    --group; one rooted anywhere else needs it,\n"
        "                                    like --dir. A ref matching more than one\n"
        "                                    session lists them on stdout and exits 2 —\n"
        "                                    candidates to choose from, not a failure.\n"
        ...
        "                                    a declared peer host; --dry-run\n"
        "                                    previews without moving anything.\n"
        "                                    Ownership moves to the peer as the\n"
        "                                    transfer's last step. A workspace\n"
        "                                    already present on the peer and owned\n"
        "                                    by this host needs --overwrite\n"
        ...
        "  camp window unbind                Put the window-creation key\n"
        "                                    (prefix+c) back to tmux's own\n"
        "                                    default, from any cwd\n"
        "\n"
        "Exit codes (camp transfer):\n"
        "  0                Every check passed — a clean verdict, or (without\n"
        "                   --dry-run) the transfer completed\n"
        ...
        "  8                The workspace already exists on the peer, owned by\n"
        "                   this host — pass --overwrite to replace it\n"
        "  9                A move phase failed before ownership moved —\n"
        "                   nothing after it ran; re-running the transfer\n"
        "                   is safe\n"
        "  10               A move phase failed AFTER ownership had already\n"
        "                   moved to the peer — not safe to re-run; continue\n"
        "                   the work on the peer and compare the two hosts'\n"
        "                   records to confirm they disagree\n"
        "  11               Ownership moved cleanly but at least one crossed\n"
        "                   conversation's release failed — this host may\n"
        "                   still hold a resumable copy of it; see the FAILED\n"
        "                   line(s) printed for which one(s) need cleaning up\n"
        "                   by hand\n"
        "  12               Whether ownership moved is genuinely unknown, even\n"
        "                   after re-probing the peer — check the peer\n"
        "                   directly with `camp transfer-probe` before doing\n"
        "                   anything else\n"
        "\n"
        "Exit codes (camp launch):\n"
        "  0                Launched — stdout is the session id and nothing else\n"
        "  1                Refused — nothing was started and stdout is empty;\n"
