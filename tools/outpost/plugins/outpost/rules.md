...
Link a record mention in prose as `[kind/slug](<base>/records/vault/kind/slug)`, never
the bare URL. Resolve base and vault once per session: base is `LORE_RECORD_URL_BASE`,
else `record_url_base` in `config.json`, else `http://127.0.0.1:7313`; vault is `lore
vault ls`'s path column matched to the record's path, never a basename. Print the
identifier bare when the vault can't be resolved there, its path isn't a direct child of
the vaults root (`$LORE_STATE_DIR/vaults`, else `$XDG_STATE_HOME/lore/vaults`, else
