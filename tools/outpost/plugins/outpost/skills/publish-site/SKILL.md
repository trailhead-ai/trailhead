...
```
python3 <skill-dir>/publish_site.py <source-dir> <slug> \
...
2. Confirms `--vault-path` is a direct child of the vaults root (a vault
   configured with an explicit, non-standard path fails this check with an
   explicit error rather than publishing somewhere unexpected).
3. Stages the source into a temporary directory inside `<vault>/sites/`,
   then atomically renames it into place as `<vault>/sites/<slug>/`. A
   failure at any point during staging leaves no partial site directory —
   the temp directory is cleaned up and any existing target is untouched.
...
0**. If sync fails, the script exits nonzero, prints the sync error, and
states plainly that the site was published locally but **NOT synced** — so
...
On success, the script prints exactly one line — the site's local URL, in
trailing-slash form:
