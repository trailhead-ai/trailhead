...
The deterministic logic behind `/craft:shelve` lives in
`plugins/craft/scripts/handoff_capture.py` so it can be tested without driving a
live Claude Code session. Coverage:
  ...
  - degraded-file write: out-of-repo `~/.craft/handoffs/<slug>.md` carrying the
...
def test_write_degraded_handoff_lands_out_of_repo(tmp_path: Path, synthetic_repo: Path):
    handoff_dir = tmp_path / "fake-home" / ".craft" / "handoffs"
    state = hc.capture_git_state(synthetic_repo, default_branch="trunk")
...
def test_write_degraded_handoff_creates_dir_when_missing(tmp_path: Path):
    handoff_dir = tmp_path / "fresh" / ".craft" / "handoffs"
...
def test_write_degraded_handoff_slug_stays_contained_under_handoff_dir(tmp_path: Path):
    ...
    write outside ~/.craft/handoffs/ even if a malformed slug reaches the helper.
    """
