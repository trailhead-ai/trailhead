# Fixture: a fenced code block fakes a duplicate ledger entry

Synthetic fixture spec body for `ledger_gate.py` tests — not a real vault record.
The real ledger has exactly one `task/alpha` entry; the fenced block below
tries to forge a second one with the same task id, but a fenced block is
masked, so this must not falsely trigger `duplicate-ledger-task-id`.
