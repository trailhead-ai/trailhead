...
The deterministic logic behind `/craft:pickup` lives in
`plugins/craft/scripts/pickup_resume.py` so it can be tested without driving a
live Claude Code session. `/craft:pickup` resumes work shelved by
`/craft:shelve` — it surfaces the recorded git state + pickup hints and flips
the shelved note back to active. Coverage:

  - most-recent craft handoff read (symmetric with handoff's degraded write):
    picks the newest file in ~/.craft/handoffs/, returns its path + content,
...
# ---------------------------------------------------------------------------
# most-recent craft handoff read (symmetric degraded read)
# ---------------------------------------------------------------------------

def test_most_recent_returns_newest_file(tmp_path: Path):
    handoff_dir = tmp_path / ".craft" / "handoffs"
