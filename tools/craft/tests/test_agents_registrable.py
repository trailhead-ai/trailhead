...
craft is agent-centric (its reason to exist is hosting general-dev agents). An
...
def test_at_least_one_agent_ships():
    """craft's whole point is hosting agents — guard against an empty dir."""
    assert _agent_files(), "craft must ship at least one agent (the proof agent)"
