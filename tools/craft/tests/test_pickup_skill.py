"""Content contract for `plugins/craft/skills/pickup/SKILL.md`.
...
verbatim enough that a future edit can't silently drop them:
  ...
  - symmetric degradation read from ~/.craft/handoffs/ (Advocate C4).
...
SKILL = (
    ...
    / "plugins" / "craft" / "skills" / "pickup" / "SKILL.md"
)
...
def test_which_backend_declaration_present(text: str):
    ...
    assert "reading local craft handoff at" in text
...
def test_empty_state_clear_message(text: str):
    """Nothing shelved AND no craft file -> a clear nothing-to-resume message."""
