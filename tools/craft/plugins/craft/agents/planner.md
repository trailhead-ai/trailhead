...
On the slice-rooted path, when the parent carries `## Enumerated states`, produce the design
doc and record its path as the `craft/design-doc` label, following `skills/plan/SKILL.md`'s
step 6.5 (write one `## State — <name>` section per enumerated bullet, name verbatim; validate
the recorded path against the safe-value shape before substitution) — see
`skills/_shared/slice.md` for the written shapes. Producing it before Define Tasks lets the
states shape the decomposition rather than being discovered after it, the same reason
`skills/plan/SKILL.md`'s step 6.5 sits where it does. On the topic-rooted path, or when the
parent carries no `## Enumerated states`, this step does nothing.
...
otherwise destroys the section before Phase 6's close gate ever reads it. Otherwise this is the
topic-rooted path, described below.
