...
1. **Verdict** — one line: `framing-holds` | `framing-wobbles` | `framing-fails`. `framing-fails` means you believe this spec should not freeze as written.
...
3. **Premise ledger** — a table: each load-bearing premise (stated or unstated), whether it's `checked` / `uncheckable`, the evidence if checked, and what happens if it's false (`collapses` / `wobbles` / `survives`).
