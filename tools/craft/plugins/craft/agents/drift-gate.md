...
Rules:
- `PASS` — the diff delivers the task's plan section (or, for a standalone leaf, the task's payload), the executor's status claim holds, every test-contract item carries mutation evidence, and nothing blocks the next task (N/A on a standalone leaf — there is no next task, so that clause cannot withhold a `PASS`).
...
4. **Mutation evidence** — for each item in the intent document's `**Test contract:**` (or `## Test contract`), does the executor's report show mutation evidence (break, RED, restore, GREEN, empty diff)? A test-contract item with no mutation evidence is DRIFT regardless of the claimed status — DONE, DONE_WITH_CONCERNS, or otherwise — not a pass with a caveat.
