...
2. **Check prior decisions.** Your project's decision records and subsystem profiles often contain load-bearing context: why the current shape exists, what was rejected last time, what constraints aren't visible in the code.
   - Optionally dispatch a knowledge-synthesis subagent if one is configured (e.g. `lore:librarian`) to sweep prior decisions, dropped backlog items, and subsystem gotchas in one pass. **If none is configured, note in your report that the prior-art synthesis pass was skipped and results may be shallower.**
