---
name: pickup
description: Resume a shelved work chain — surface the recorded git state and pickup hints from a prior /craft:shelve. Use for /craft:pickup, /craft:pickup <slug>, "pick up where I left off", "resume the work on X".
---

# Pickup

Resume work shelved by `/craft:shelve` — locate the shelved session note (or local craft handoff file), surface its `## Pickup hints` and recorded git state, and flip the note back to active. A `<slug>` argument targets a specific shelved chain; with no argument, pickup surfaces the most recent one.
...
- Otherwise (lore absent, `$LORE_VAULT` unset, or `lore stats` non-zero) → **degraded**. Announce: **"lore unavailable — reading local craft handoff at `~/.craft/handoffs/`"**.
...
Read from the SAME location handoff wrote to — `~/.craft/handoffs/` (out of any repo). Use the helper:

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/pickup_resume.py"                 # most-recent craft handoff
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/pickup_resume.py" --slug "<slug>" # a specific one
```

With no slug it surfaces the **most-recent** craft handoff file and prints its path — never a lore lookup that silently returns nothing.
...
There is no status to flip — the craft handoff file is a plain artifact. The hints are surfaced; the file stays in place for re-reading.

## Empty state

If **nothing is shelved AND no craft handoff file exists**, say so plainly — **"nothing to resume"** — not an empty list or a traceback. On the lore path `lore shelved` already prints a clear nothing-shelved message; on the degraded path the helper prints `nothing to resume — no craft handoff file found`.
