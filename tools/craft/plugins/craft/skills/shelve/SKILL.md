---
name: shelve
description: Record your current git state and shelve a session note so a future session can resume. Use for /craft:shelve, "shelve this work", "hand off this work", "save my progress", "I'm done for now".
---

# Shelve

Capture a read-only snapshot of your git state, compose pickup hints, and shelve a session note (via the `lore` CLI when available, otherwise a local craft file). A future `/craft:pickup` resurfaces it.
...
- Otherwise (lore absent, `$LORE_VAULT` unset, or `lore stats` non-zero) → **degraded**. Announce: **"lore unavailable — writing local craft handoff at `~/.craft/handoffs/<slug>.md`"**.
...
```bash
HINTS_TMP=$(mktemp /tmp/craft-handoff-hints.XXXXXX)
printf '%s\n' "$HINTS" > "$HINTS_TMP"
...
This writes `~/.craft/handoffs/<slug>.md` (creating `~/.craft/handoffs/` if missing) containing the hints + captured git state.

## Step 5 — Confirm + pickup discoverability

Close with the landing location (vault path or `~/.craft/handoffs/<slug>.md`) and the resume hint:

> To resume: `/craft:pickup [slug]`
