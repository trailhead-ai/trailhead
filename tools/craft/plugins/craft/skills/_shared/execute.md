...
The Bash tool auto-backgrounds any command that runs past ~120s, so on a suite whose measured runtime exceeds that, "run it in the foreground" is impossible to obey without a stated override in the dispatch. Obey it by naming a concrete timeout value above the suite's measured runtime — 600000ms is the working default when the runtime is unmeasured — never by starting the suite as a background job and ending the turn waiting on it. Separately, require the executor to commit and report its own result in the same turn as its final test run rather than parking on a monitor. When the suite's measured runtime exceeds the Bash tool's 600000ms ceiling, no single timeout value can satisfy the mandate: split the run into a scoped subset of the suite that fits under the ceiling, or dispatch against a scoped suite and rely on the controller's own full-suite gate ([Phase 1](#phase-1-test-runner-gate)) to catch what the scoped run couldn't.
...
**Each phase, at the tick, upserts the phase boundary onto the parent task record via `lore record update`, in the same write that records the tick itself:** `lore record update task/<parent-name> --vault <elected-vault> --diff --label craft/phase-boundary=<sha>`, piping the tick's unified diff on stdin, `<sha>` being `HEAD` **after** that phase's commits land — so the label always names the tree the last completed phase left. A phase that commits nothing rewrites the same value onto the label; the write is idempotent. **The tick and the boundary must land as one atomic write, never a body write followed by a separate label write:** a crash landing between the two would leave a ticked phase whose recorded boundary still names the *previous* phase's tree, and the resume precondition below would then revert a dirty tree to it and discard the completed phase's commits. If the write fails — vault lock, permission, an unforeseen reserved-key collision — the phase logs the failure and retries the write before proceeding; a phase is never reported ticked while its boundary write has failed.

**Re-entering a resume branch requires a clean-working-tree precondition, but which guarantee is available differs by branch — the two `in-progress` cases at [Resuming a run](#resuming-a-run):**

- **Re-entering any end phase on resume** — at least one `## End Phases` line is already ticked, so a `craft/phase-boundary` label is guaranteed to exist by construction. A dirty tree found here — staged, unstaged, or untracked changes from a mid-mutation crash — is **reverted to the last recorded phase boundary**, read structurally via `lore record show task/<parent-name> --vault <elected-vault> --json` → `.sidecar.labels["craft/phase-boundary"]`, but only once the value clears validation: it must match `^[0-9a-f]{7,40}$` **and** resolve to a commit reachable from the task branch (`git merge-base --is-ancestor <sha> HEAD` exits 0 on the checked-out task branch). Neither check is optional — the same untrusted-input rule stated near the top of this document governs this label like any other vault-sourced value substituted into a command shown in this document, and this one feeds a destructive revert. **Absent or invalid fails closed:** if the label is missing, does not match that shape, or does not resolve on the branch, resume stops and reports rather than reverting to a guessed target — never substituted into the revert command. This is what prevents footprint corruption: `footprint_guard.py` unions live working-tree drift into its check, so a stray uncommitted edit from a crashed mutation would otherwise false-positive the guard or, worse, get folded into a later commit. Never re-dispatch a phase onto a dirty tree.
- **The mid-build resume branch** — [No ticked phase line](#resuming-a-run), before any phase has ticked — so no boundary label exists yet by construction: Phase 1 is what first writes one. Treating that absence as the fail-closed stop above would deadlock every dirty mid-build resume, since the label the stop waits for cannot exist until the build produces its first tick. Instead, the tree is made clean by discarding uncommitted and untracked changes against the current `HEAD` before the executor is re-dispatched. Reverting to `base` (the pre-execution sha recorded in `## End Phases`) would be wrong here: reverting to it would discard whatever slice commits the build has already landed, and `base` predates all of them. Discarding drift against `HEAD` instead destroys no committed work and needs no label to be well-defined, so it is not a fail-closed stop.
