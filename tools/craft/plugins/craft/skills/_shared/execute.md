...
Sanctioned method tokens are exactly `automated-assertion`, `design-doc-review`, and `manual-check`; evidence text is required and never empty. Recording as evidence arrives — rather than composing the section at Phase 6 from nothing — is the point: a section written moments before the gate reads it certifies nothing.
...
Without a matching attestation the gate refuses that criterion's discharge. **Residual limit, stated plainly:** this raises the cost of a fabricated manual check from writing one sentence to fabricating a two-part record; it is a ritual boundary, not proof of human review — a run determined to forge both halves still can.
