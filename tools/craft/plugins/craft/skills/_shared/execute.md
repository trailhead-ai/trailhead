...
**Run Metrics totals.** At close, total the `## Run Metrics` block's rows into a final run-total row: slices, total dispatches, dispatches-per-slice, and end-to-end wall clock from the first dispatch to the last terminal status. The completion report's metrics line names lessons written and lessons consumed this run, alongside the existing per-phase outcomes, so the operator sees what the loop taught itself where they already read the run's outcome.
