...
Here `<name>` is the parent task's own `craft/subsystems` label value — the same value the postmortem writes on the lesson, not a repo, area, or branch name. Substituting anything else yields a conjunction that matches nothing and is indistinguishable from genuine absence. **When the task carries no `craft/subsystems` label** — planning writes it only conditionally and refine never writes it, so a standalone run routinely has none — drop the `label.craft.subsystems:` term and run the query as `kind:lesson label.craft.dispatch-lesson:executor`, then record the run as having queried unscoped; the postmortem's write drops the same label on the same condition, so the two halves stay symmetric instead of one of them inventing a value. The contract this query and the postmortem's write share is `kind lesson + craft/dispatch-lesson=executor + craft/subsystems=<name>`; both sides are pinned against one canonical fixture so neither can be renamed alone.
...
Personal-vault lessons are fenced `layer="shared"` too — deliberately conservative, and not a contradiction of the retrieval section's trusted-unfenced-channel framing: the fence marks text crossing a dispatch boundary into another agent's prompt, not the vault it came from.
