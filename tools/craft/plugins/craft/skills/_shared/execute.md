...
Sanctioned method tokens are exactly `automated-assertion`, `design-doc-review`, and `manual-check`; evidence text is required and never empty. Recording as evidence arrives — rather than composing the section at Phase 6 from nothing — is the point of anchoring the producer here, at Phase 5, immediately before Phase 6's close: it gives each observation a natural place to land as its evidence actually appears, rather than a place to compose a plausible-looking section at the last moment. **This is a convention the anchor makes natural to follow, not a guarantee the gate can check:** nothing observes or enforces *when* a line was written, so a run could still author the whole section in one pass right before Phase 6 reads it, indistinguishable on the wire from one recorded as evidence arrived.
