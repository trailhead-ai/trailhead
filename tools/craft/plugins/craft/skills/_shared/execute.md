...
Record phase progress as an `## End Phases` checklist appended to the **parent task body** (via `lore record update`), one line per phase, ticked as each completes. This is what makes the pipeline resumable if context breaks mid-run: on resume, read the checklist and re-enter at the first unticked phase. At entry to the end-phase pipeline, record the run's `base` (the pre-execution SHA, fixed once per [After All Slices](#after-all-slices)) into the `## End Phases` checklist, so a resumed run can recover it.
