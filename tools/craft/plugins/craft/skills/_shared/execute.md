...
**Each phase, at the tick, upserts the phase boundary onto the parent task record:** `lore record update task/<parent-name> --vault <elected-vault> --label craft/phase-boundary=<sha>`, `<sha>` being `HEAD` **after** that phase's commits land — so the label always names the tree the last completed phase left. A phase that commits nothing rewrites the same value onto the label; the write is idempotent.

**Re-entering any end phase on resume requires a clean-working-tree precondition.** A dirty tree found on resume — staged, unstaged, or untracked changes from a mid-mutation crash — is **reverted to the last recorded phase boundary**, read structurally via `lore record show task/<parent-name> --vault <elected-vault> --json` → `.sidecar.labels["craft/phase-boundary"]`, before any re-dispatch. **Absent fails closed:** if that label is not present, resume stops and reports rather than reverting to a guessed target. This is what prevents footprint corruption: `footprint_guard.py` unions live working-tree drift into its check, so a stray uncommitted edit from a crashed mutation would otherwise false-positive the guard or, worse, get folded into a later commit. Never re-dispatch a phase onto a dirty tree.
