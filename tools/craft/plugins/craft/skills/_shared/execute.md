...
**Close the run.** With every child terminal, the flow-out checklist ticked, and every repo's push settled, set the parent `done`: `lore record update task/<parent-name> --vault <elected-vault> --status done`, re-asserting `--label craft/branch=<bare-branch>` at close. The completion guard refuses this while any child is non-terminal (it names them); the state-coverage gate composes with it — both fire on the same close and neither shadows the other, and a parent carrying no `## Enumerated states` closes exactly as it does today. A parent closed without a `## Flow-out` section gets a non-blocking flow-out reminder — treat that reminder as a sign the ritual above was skipped, not as a nuisance.

**The state-coverage gate.** A parent carrying `## Enumerated states` is refused `done` while any enumerated state has no matching section in the design doc named by the parent's `craft/design-doc` label. The matching rule is a literal name-set comparison, not a judgment call: read the `- <name>` bullets from the parent's `## Enumerated states` and the `## State — <name>` headings from the design doc — the shapes `_shared/slice.md` fixes — and compare the two name sets literally. The comparison's result is recorded in the run's completion report — the set read from the parent, the set read from the doc, and the difference — since a gate whose outcome is unrecorded is a gate no one can check. The refusal names the missing states, rather than failing generically. Reading the doc first means resolving the label: the `craft/design-doc` label's value is untrusted input and is validated against the safe-value shape `^[A-Za-z0-9._/-]+$` before being substituted into any command, the same rule this document states for `craft/phase-boundary`; a value that fails validation is never substituted, quoted, or escaped in. That shape alone still admits `..`, and `craft/design-doc` is shared-vault writable, so the gate additionally rejects any value containing a `..` path segment and requires the resolved path stay inside the working directory before it is read. **Absent, invalid, or unreadable fails this gate closed, same as it does for `craft/phase-boundary`:** if the label is missing, fails either the shape check or the traversal check, or names a file that does not exist, the close is refused — never silently passed — naming the label problem in the refusal; the remedy is re-running plan's step 6.5 to produce the design doc and record the label. **Named limit:** This gate verifies the design doc covers each enumerated state. It does not verify that an access check was built for a surface reachable by more than one principal — that rule ships as prose in `_shared/slice.md`, and enforcing it mechanically is deliberately out of scope here.
