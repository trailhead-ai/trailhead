...
**Scoped per-slice runs are the default, not an escape hatch.** Name the scoped command in the dispatch — this is what the executor runs after every red/green cycle, whether or not the entire suite would fit under the timeout. The controller's own Phase 1 full-suite gate is not licence for per-slice full runs: per-slice runs stay scoped throughout the build, and the entire suite runs exactly once, at the controller's [Phase 1](#phase-1-test-runner-gate) gate after all slices land.
