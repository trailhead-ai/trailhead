...
Execute a plan slice-by-slice — or a standalone task as its own single slice — in an
...
the task-shape branch, the per-slice loop, the end-of-run phase pipeline, and the
...
task-shape branch (plan vs. standalone), the per-slice Loop, the After All Slices
