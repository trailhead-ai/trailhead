"""Deterministic helpers for the `/craft:shelve` dev ritual.

`/craft:shelve` records generic git state and shelves a session note so a
...
def write_degraded_handoff(
    ...
    ``~/.craft/handoffs/`` (OUT of any repo), so captured git state never leaks
    ...
    body = (
        f"# Craft handoff — {slug}\n\n"
        "_lore unavailable — this is a local craft handoff (out of any repo)._\n\n"
        "## Pickup hints\n\n"
        f"{hints.strip()}\n\n"
        "## Captured git state\n\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="directory for the degraded handoff file (default: ~/.craft/handoffs)",
    )
