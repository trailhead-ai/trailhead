#!/usr/bin/env bash
# install-hooks.sh — idempotently install the craft leak-gate as a pre-commit
...
MARKER="# craft-leak-gate-installed"

# Already installed by us?
if [ -f "$TARGET" ] && grep -qF "$MARKER" "$TARGET" 2>/dev/null; then
    echo "craft leak gate already installed at $TARGET"
    exit 0
fi

if [ -e "$TARGET" ]; then
    ORIGINAL_HOOK="$GIT_HOOKS_DIR/pre-commit-before-craft"
    cp "$TARGET" "$ORIGINAL_HOOK"
    chmod +x "$ORIGINAL_HOOK"
    echo "craft: existing pre-commit hook saved to $ORIGINAL_HOOK"

    cat > "$TARGET" <<EOF
#!/usr/bin/env bash
$MARKER
# Chain: run the original pre-commit hook, then the craft leak gate.
...
#!/usr/bin/env bash
$MARKER
# Craft leak gate — blocks commits that would ship a private string.
...
chmod +x "$TARGET"
echo "Installed craft leak gate at $TARGET"
