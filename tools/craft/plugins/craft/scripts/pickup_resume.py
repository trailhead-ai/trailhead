"""Deterministic helpers for the `/craft:pickup` dev ritual.

`/craft:pickup` resumes work shelved by `/craft:shelve`: it surfaces the
...
lives here:
  ...
  - ``most_recent_handoff``  symmetric degraded read — locate the newest
    craft handoff file (or one named by slug) under the handoff dir.
...
``~/.craft/handoffs/<slug>.md`` (out of any repo), and pickup reads from the
...
def _default_handoff_dir() -> Path:
    return Path.home() / ".craft" / "handoffs"
...
def most_recent_handoff(
    ...
    """Return the relevant craft handoff file, or None.
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="directory holding craft handoff files (default: ~/.craft/handoffs)",
    )
    ...
    if found is None:
        print("nothing to resume — no craft handoff file found")
        return 0
