...
- Spec this slice belongs to: `spec/dock-scheduling-windows` (vault: `harborlight`, status
  `ready`).
- Slice-loop marker on the spec: `craft/slice-loop` reads *not* `complete` — the slice loop has not
  reported this spec closed out; further slices remain to be chosen, planned, or built.
- This review closes its own job, not the spec's lifecycle.
