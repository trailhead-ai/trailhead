...
This fixture is byte-identical to `positive.md` except for AC3's wording and the `## Maturity`
stamp (and the rendered block the stamp changes) — the target repository is stamped
`production` here, so the variable under test is isolated to the resolved level. AC3 no longer
calls the target a "disposable-state prototype" — `positive.md`'s AC3 wording contradicts a
`production` stamp, so this fixture's AC3 was reworded to state the same waived-preservation
requirement without asserting the target's maturity level itself.
