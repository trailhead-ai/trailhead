...
class TestAgentCopyOpsIncluded:
    ...
    def test_lore_recall_lore_librarian_in_plan(self, tmp_path):
        """lore recall capability declares agents/librarian.md; must be in plan."""
        m = load_manifest(_LORE_MANIFEST)
        dest = tmp_path / "dest"
        plan = compose_plan(m, {"recall"}, dest)
        dest_paths = {op.dest for op in plan.ops}
        assert dest / "agents/librarian.md" in dest_paths
...
class TestAgentApplyLanding:
    ...
    def test_lore_librarian_lands_after_apply(self, tmp_path):
        """lore recall → agents/librarian.md lands in dest."""
        m = load_manifest(_LORE_MANIFEST)
        dest = tmp_path / "dest"
        plan = compose_plan(m, {"recall"}, dest)
        apply_plan(plan, mode="copy")
        agent_path = dest / "agents" / "librarian.md"
        assert agent_path.exists()
        assert agent_path.is_file()
    ...
    def test_agent_content_preserved(self, tmp_path):
        ...
        plan = compose_plan(m, {"recall"}, dest)
        apply_plan(plan, mode="copy")
        src_agent = m.plugin_root / "agents" / "librarian.md"
        dest_agent = dest / "agents" / "librarian.md"
...
class TestSubsetEnforcement:
    def test_minimal_lore_plan_has_no_recall_agents_when_recall_absent(self, tmp_path):
        """Composing lore WITHOUT recall: librarian agent must not appear in plan.

        M-5 fix: shared-vaults has agents=[], so testing its absence is vacuous.
        Instead assert that recall's agent (librarian.md, non-empty) is absent
        ...
        plan = compose_plan(m, {"capture"}, dest)
        dest_paths = {op.dest for op in plan.ops}
        # recall has the librarian agent — must not leak
