...
class TestForwardCheckPositive:
    ...
    def test_real_skill_claim_passes(self):
        ...
        # lore has skills/flush (not finish/checkpoint)
...
class TestBuildRealAnchorSet:
    ...
    def test_camp_anchors_cover_its_skills_and_no_agents(self):
        """camp is a CLI + hooks tool: no subagents, two skills.

        `concierge` and `fork`. Worktree orchestration stays operator-facing
        (README) — the workspace exists before the harness opens — so it is not
        anchored as a skill.
        """
        anchors = build_real_anchor_set()
        assert anchors["camp"]["skills"] == {"skills/concierge", "skills/fork"}
