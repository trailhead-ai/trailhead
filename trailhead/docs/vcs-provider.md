...
| `pr.merge(pr_pairs, manifest_path)` | `gh pr merge <n> --merge --author-email …`; `git push origin --delete -- <branch>` | `glab mr merge <iid>` → `PUT /projects/:id/merge_requests/:iid/merge`; branch delete via `--remove-source-branch` | The ordered-merge safety gate and the `auto_merge` fail-closed gate are both provider-orthogonal (operate on `[release]` config + manifest, before any platform call). |
