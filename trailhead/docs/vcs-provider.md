...
| `pr.merge(pr_pairs, manifest_path)` | `gh pr merge <n> --merge\|--squash\|--rebase --author-email …` (method from `[release].merge_method` — `merge`/`squash`/`rebase`/`automatic`, default `automatic`, resolved per PR); `git push origin --delete -- <branch>` | `glab mr merge <iid> [--squash\|--rebase]` → `PUT /projects/:id/merge_requests/:iid/merge`; branch delete via `--remove-source-branch` | The ordered-merge safety gate, the `auto_merge` fail-closed gate, and the `merge_method` fail-closed gate are all provider-orthogonal (operate on `[release]` config + manifest, before any platform call). Automatic selection (`resolve_merge_strategy`) is a pure function over the repository's permitted strategies and, where rebase is forbidden with more than one strategy permitted, the PR's commit series — provider-orthogonal by construction, since both inputs are already-normalized values rather than raw platform JSON. |
