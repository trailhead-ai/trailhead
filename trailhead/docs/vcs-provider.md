...
`trailhead/vcs/` defines one provider-agnostic interface (`Provider`) with three
namespaced surfaces and ships a single `GitHubProvider` backend. The library is
the merge seam that the `portage` plugin builds on. Every `gh`/`git` call routes
through the injectable runner (`trailhead/vcs/runner.py`, `shell=False`), so
tests inject a stub and never touch the network.
...
1. Implement a `Provider` subclass (e.g. `GitLabProvider`) whose `repos`/`pr`/
   `ci` surfaces fulfil the same method contract, routing every `glab`/REST
   call through `trailhead.vcs.runner` (`shell=False`).
...
The `Provider` interface (`trailhead/vcs/interface.py`) exposes three surfaces:
...
| `pr.merge(pr_pairs, manifest_path, operator_directed)` | `gh pr merge <n> --merge\|--squash\|--rebase --author-email …` (method from `[release].merge_method` — `merge`/`squash`/`rebase`/`automatic`, default `automatic`, resolved per PR); `git push origin --delete -- <branch>` | `glab mr merge <iid> [--squash\|--rebase]` → `PUT /projects/:id/merge_requests/:iid/merge`; branch delete via `--remove-source-branch` | The ordered-merge safety gate, the `auto_merge` fail-closed gate (passed only by `operator_directed` when auto_merge is off), and the `merge_method` fail-closed gate are all provider-orthogonal (operate on `[release]` config + manifest, before any platform call). Automatic selection (`resolve_merge_strategy`) is a pure function over the repository's permitted strategies and, where rebase is forbidden with more than one strategy permitted, the PR's commit series — provider-orthogonal by construction, since both inputs are already-normalized values rather than raw platform JSON. |
